...
pub async fn start(ctx: Context<'_>) -> Result<(), Error> {
    ...
    if settings.lorax_channel.is_none() {
        ctx.say("❌ Please set a Lorax channel first using `/lorax config channel`").await?;
        return Ok(());
    }

    // Start new event using task system
    let mut task = LoraxEventTask::new(guild_id, ctx.data().dbs.lorax.clone());
    task.start_event(guild_id, settings, &ctx.serenity_context()).await;

    ctx.say("🎉 The Lorax event has begun! Let the naming commence!").await?;
    Ok(())
}
...
pub async fn end(ctx: Context<'_>) -> Result<(), Error> {
    ...
    match task.end_event(guild_id, &ctx.serenity_context()).await {
        Ok(_) => {
            ctx.say("🛑 The Lorax event has been ended. Thanks for participating!").await?;
        },
        Err(e) => {
...
pub async fn status(ctx: Context<'_>) -> Result<(), Error> {
    ...
    let event = ctx.data().dbs.lorax.get_event(guild_id).await
        .ok_or("No active Lorax event")?;

    let status_msg = match &event.stage {  // Match on reference to avoid moving
        ...
        stage @ (LoraxStage::Voting | LoraxStage::Tiebreaker(_)) => {
            ...
            if event.current_trees.len() > 3 {
                msg.push_str(&format!("... and {} other entries\n", event.current_trees.len() - 3));
            }
        ...
        LoraxStage::Completed => {
            let mut msg = format!(
                "✨ Event completed\nWinner: {}",
                event.current_trees.first().unwrap_or(&"Unknown".to_string())
            );

            // Show top 3 with medals and submitters
            msg.push_str("\n\n🏆 Final Results:");
            for (i, tree) in event.current_trees.iter().take(3).enumerate() {
                let medal = match i {
                    0 => "🥇",
            ...
            if event.current_trees.len() > 3 {
                msg.push_str(&format!("\n\n... and {} other runner-up{}!", 
                    event.current_trees.len() - 3,
                    if event.current_trees.len() - 3 == 1 { "" } else { "s" }
                ));
            }
            msg
        },
...
pub async fn force_advance(ctx: Context<'_>) -> Result<(), Error> {
    ...
    // Don't allow advancing completed/inactive events
    if matches!(event.stage, LoraxStage::Completed | LoraxStage::Inactive) {
        ctx.say("❌ Cannot advance a completed or inactive event.").await?;
        return Ok(());
    }
    
    let mut updated_event = event.clone();
    let task = LoraxEventTask::new(guild_id, ctx.data().dbs.lorax.clone());
    
    // Use the same advancement logic as the task system
    task.advance_stage(&ctx.serenity_context(), &mut updated_event).await;
    
    // Only update database if the event is still active
    if !matches!(updated_event.stage, LoraxStage::Inactive) {
        ctx.data().dbs.lorax.update_event(guild_id, updated_event).await;
    }
...
pub async fn stats(ctx: Context<'_>) -> Result<(), Error> {
    ...
    let task = LoraxEventTask::new(guild_id, ctx.data().dbs.lorax.clone());
    let duration = task.calculate_stage_duration(&event);
    let end_time = event.start_time + duration;

    let stage_info = match &event.stage {  // Match on reference to avoid moving
        LoraxStage::Submission => format!(
            "🌿 **Submission Phase**\n\
            Submissions: {}\n\
            Most Recent: {}",
            event.tree_submissions.len(),
            event.tree_submissions.iter()
                .last()
        ...
        stage @ (LoraxStage::Voting | LoraxStage::Tiebreaker(_)) => {
            ...
            msg.push_str(&format!(
                ...
                (event.tree_votes.len() as f32 / (event.tree_submissions.len().max(1) - 1) as f32 * 100.0) as u32
            ));

            // Add top 3 with medals and submitters
            for (i, tree) in event.current_trees.iter().take(3).enumerate() {
                let medal = match i {
                    0 => "🥇",
                    1 => "🥈",
...
pub async fn duration(
    ...
    let mut event = ctx.data().dbs.lorax.get_event(guild_id).await
        .ok_or("No active Lorax event")?;

    let task = LoraxEventTask::new(guild_id, ctx.data().dbs.lorax.clone());

    // Calculate new duration
    let current_duration = task.calculate_stage_duration(&event);
    let new_duration = (current_duration as i64 + (minutes * 60)) as u64;
    
    // Update the start time to reflect new duration while maintaining same end time
    ...
    // Update the stage message if it exists
    if let Some(channel_id) = event.settings.lorax_channel {
        ...
        // Update original stage message timestamp if it exists
        if let Some(msg_id) = match event.stage {
            ...
            if let Ok(mut message) = channel.message(&ctx.serenity_context().http, msg_id).await {
                ...
                let _ = message.edit(&ctx.serenity_context().http, EditMessage::new().content(new_content)).await;
            }
        }
    }

    ctx.data().dbs.lorax.update_event(guild_id, event).await;
    ctx.say(format!("⏳ Stage duration adjusted by {} minutes.", minutes)).await?;
    Ok(())
}
...
#[command(slash_command, guild_only, ephemeral, required_permissions = "MANAGE_MESSAGES")]
pub async fn submissions(ctx: Context<'_>) -> Result<(), Error> {
    ...
    let mut submissions: Vec<_> = event.tree_submissions
        .iter()
...
pub async fn votes(ctx: Context<'_>) -> Result<(), Error> {
    ...
    let mut votes: Vec<_> = event.tree_votes
        .iter()
...
pub async fn remove_submission(
    ...
    // Remove any votes for this tree
    event.tree_votes.retain(|_, voted_tree| voted_tree != &tree);

    ctx.data().dbs.lorax.update_event(guild_id, event).await;
    ctx.say(format!("🗑️ Removed submission \"{}\" and any related votes.", tree)).await?;
    Ok(())
}
...
pub async fn remove_vote(
    ...
    if event.tree_votes.remove(&user.id.get()).is_some() {
        ctx.data().dbs.lorax.update_event(guild_id, event).await;
        ctx.say(format!("🗑️ Removed vote from {}.", user.mention())).await?;
    } else {
        ctx.say(format!("❌ {} has not voted.", user.mention())).await?;
    }
    Ok(())
}
